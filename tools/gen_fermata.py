...
CATALOG = BASE / "Library/data/kba_catalog/records"

DEFER_KEYWORDS = ['{DEFER}', 'deferred', 'next stop', 'planned maintenance',
...
def classify_group(ptype, emcat, title, affected_products):
    ...
    if ptype == 'bug_software':
        io_keywords = ['i/o', 'cioc', 'eioc', 'wioc', 'rio', 'scanner', 'wireless i/o', 'zone 2', 'controller']
...
for row in filtered:
    ...
    groups[grp][site].append({
        'kba': kba, 'nodes': nodes, 'desc': desc, 'workaround': ws,
    })
