...
from tools.kba_reporter.config import (
    PROJECT_ROOT, OWNERS_INBOX_DIR, HANDOFF_DIR,
    LOG_FILE, ANALYSIS_MAX_AGE_DAYS,
)
...
def main(
    ...
    max_age: int = typer.Option(ANALYSIS_MAX_AGE_DAYS, "--max-age", help="Giorni max per analysis fresca."),
    ...
    console.print(f"[green][OK][/green] Lista patch     -> {patch_output.relative_to(PROJECT_ROOT)}")
    ...
    console.print(f"[green][OK][/green] Brief WIP       -> {brief_output.relative_to(PROJECT_ROOT)}")
    ...
    if mancanti:
        ...
        console.print()
        console.print(
            "I seguenti KBA non sono nel catalogo né in Library/documents/.\n"
        ...
        console.print("  3. Analizza:  python -m tools.consulto --prompt Team/Prompts/kba/analisi-rischio-kba.md \\")
        console.print("                  --input Library/documents/<slug>.md \\")
        console.print("                  --output Library/Fucina/Handoff/kba_batch/ --provider grok")
        console.print("  4. Indicizza: python -m tools.kba_indexer index --input Library/Fucina/Handoff/kba_batch/")
    ...
    console.print(f"[green][OK][/green] KBA discussione -> {discussion_output.relative_to(PROJECT_ROOT)}")
