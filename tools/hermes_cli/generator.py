...
def _backup_path(original: Path) -> Path:
    """Genera un path di backup in /tmp/hermes-cli-backup/."""
    today = date.today().isoformat()
    backup_dir = Path("/tmp/hermes-cli-backup") / today
    backup_dir.mkdir(parents=True, exist_ok=True)
    return backup_dir / original.name


def _backup_file(file_path: Path) -> Path | None:
    """Copia un file in /tmp/hermes-cli-backup/. Torna il path backup o None."""
    try:
...
def create_handoff(
    ...
    Args:
        ...
        mittente: Nome mittente (default: hermes).
