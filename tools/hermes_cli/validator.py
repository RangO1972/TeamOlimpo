...
def validate_scratchpad(sp: Scratchpad) -> Scratchpad:
    ...
    if not isinstance(raw_tasks, list):
        ...
        sp.warnings.append({"type": "empty_list", "field": "active_tasks", "description": "active_tasks è vuoto"})

    for i, t in enumerate(sp.tasks):
        if not t.id:
            sp.warnings.append({"type": "missing_field", "field": f"active_tasks[{i}].id", "description": f"Task {i} senza id"})
    ...
    if ms is None:
        sp.warnings.append({"type": "missing_field", "field": "members_status", "description": "Campo 'members_status' assente nel frontmatter"})
    elif not isinstance(ms, dict):
        sp.warnings.append({"type": "wrong_type", "field": "members_status", "description": "members_status non è un mapping valido"})
    ...
    if not isinstance(raw_decisions, list):
        ...
        sp.warnings.append({"type": "empty_list", "field": "decisions", "description": "decisions è vuoto"})

    for i, d in enumerate(sp.decisions):
        if not d.id:
            sp.warnings.append({"type": "missing_field", "field": f"decisions[{i}].id", "description": f"Decisione {i} senza id"})
        if not d.date:
            sp.warnings.append({"type": "missing_field", "field": f"decisions[{i}].date", "description": f"Decisione {d.id} senza date"})
        if not d.description:
            sp.warnings.append({"type": "missing_field", "field": f"decisions[{i}].description", "description": f"Decisione {d.id} senza description"})

    if "last_updated" not in sp.raw:
        sp.warnings.append({"type": "missing_field", "field": "last_updated", "severity": "minor", "description": "Campo 'last_updated' mancante (opzionale)"})
...
def validate_handoff_fields(fm: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for campo in required:
        if campo not in fm or fm[campo] is None:
            errors.append({"type": "missing_field", "field": campo, "description": f"Campo obbligatorio mancante: '{campo}'"})

    if "mittente" in fm and fm["mittente"]:
        val = str(fm["mittente"]).lower()
        if val not in MEMBRI:
            warnings.append({"type": "invalid_value", "field": "mittente", "actual": val, "description": f"Mittente '{val}' non è un membro valido"})

    if "destinatario" in fm and fm["destinatario"]:
        val = str(fm["destinatario"]).lower()
        if val not in MEMBRI and val != "team":
            warnings.append({"type": "invalid_value", "field": "destinatario", "actual": val, "description": f"Destinatario '{val}' non è un membro valido o 'team'"})

    if "tipo" in fm and fm["tipo"]:
        val = str(fm["tipo"]).lower()
        if val not in TIPI_HANDOFF:
            warnings.append({
                "type": "invalid_enum", "field": "tipo", "actual": val,
    ...
    if "stato" in fm and fm["stato"]:
        ...
        if val not in STATI_HANDOFF:
            warnings.append({
                "type": "invalid_enum", "field": "stato", "actual": val,
    ...
    if "priorita" in fm and fm["priorita"]:
        ...
        if val not in PRIORITA_VALIDE:
            warnings.append({
                "type": "invalid_enum", "field": "priorita", "actual": val,
...
def validate_handoff_file(path: Path) -> HandoffValidation:
    ...
    logger.debug(f"Validato handoff: {path.name} — naming_valid={hv.naming_valid}, errors={len(hv.errors)}, warnings={len(hv.warnings)}")
    return hv
