...
def session(
    ...
    if act == "observe":
        ...
        if not type or type not in valid_types:
            return json.dumps(
                {"error": f"Invalid type '{type}'. Use: {', '.join(sorted(valid_types))}."}
            )
...
def task(
    ...
    valid_actions = {"create", "query", "update", "log", "summary", "export", "compress"}
    if act not in valid_actions:
        return json.dumps(
            {"error": f"Invalid action '{act}'. Use: {', '.join(sorted(valid_actions))}."}
        )
    ...
    if act == "update":
        ...
        if not sts:
            return json.dumps({"error": "'new_status' required for action=update"})
            ...
            return _j(
                {
                    ...
                    "warning": f"Task already in state '{sts}'.",
                }
            )

        try:
            StateMachine.validate_transition(old_status, sts)
        except ValueError as e:
            return json.dumps({"error": str(e)})

        result = store.update_task_status(task_id=tid, new_status=sts, note=note)
        return _j(result)
