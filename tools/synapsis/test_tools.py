...
class TestTaskLifecycle:
    ...
    def test_create_invalid_initial_status(self, store: SynapsisStore) -> None:
        ...
        result = _parse(server_module.task(act="create", desc="Test", status="completed"))
        assert "error" in result
    ...
    def test_query_filter_by_status(self, store: SynapsisStore) -> None:
        """Filter by status."""
        _init_task(store, "T-QUERY-S1", status="pending")
        _init_task(store, "T-QUERY-S2", status="completed")
        result = _parse(server_module.task(act="query", status="completed"))
        assert len(result) == 1
    ...
    def test_update_valid_transition(self, store: SynapsisStore) -> None:
        ...
        _init_task(store, "T-STATUS-001", status="pending")
        result = _parse(server_module.task(act="update", tid="T-STATUS-001", sts="in_progress"))
        assert result["new_status"] == "in_progress"

    def test_update_same_status_noop(self, store: SynapsisStore) -> None:
        """Same status returns warning."""
        _init_task(store, "T-STATUS-002", status="pending")
        result = _parse(server_module.task(act="update", tid="T-STATUS-002", sts="pending"))
    ...
    def test_update_invalid_transition(self, store: SynapsisStore) -> None:
        ...
        _init_task(store, "T-STATUS-003", status="completed")
        result = _parse(server_module.task(act="update", tid="T-STATUS-003", sts="pending"))
        assert "error" in result

    def test_update_with_note(self, store: SynapsisStore) -> None:
        """Transition with note."""
        _init_task(store, "T-STATUS-004", status="pending")
        result = _parse(
            server_module.task(act="update", tid="T-STATUS-004", sts="completed", note="All done")
        )
        assert result["new_status"] == "completed"

    def test_update_nonexistent_task(self, store: SynapsisStore) -> None:
        """Nonexistent task returns error."""
        result = _parse(server_module.task(act="update", tid="T-NOEXIST-001", sts="completed"))
        assert "error" in result
    ...
    def test_log_handoff_ref(self, store: SynapsisStore) -> None:
        ...
        assert result["event_type"] == "handoff_ref"
    ...
    def test_summary_all(self, store: SynapsisStore) -> None:
        ...
        _init_task(store, "T-SUM-001", status="pending")
        _init_task(store, "T-SUM-002", status="in_progress")
        _init_task(store, "T-SUM-003", status="completed")
        result = _parse(server_module.task(act="summary"))
        assert result["total"] >= 3
        assert result["status"]["completed"] >= 1  # _j compressed: by_status -> status
    ...
    def test_summary_by_owner(self, store: SynapsisStore) -> None:
        ...
        _init_task(store, "T-SUM-O1", owner="efesto", status="pending")
        _init_task(store, "T-SUM-O2", owner="atena", status="completed")
...
class TestStateMachine:
    ...
    def test_all_valid_transitions(self) -> None:
        ...
        assert StateMachine.is_valid_transition("pending", "in_progress")
        assert StateMachine.is_valid_transition("pending", "completed")
        assert StateMachine.is_valid_transition("pending", "cancelled")
        assert StateMachine.is_valid_transition("pending", "blocked")
        assert StateMachine.is_valid_transition("pending", "standby")
        assert StateMachine.is_valid_transition("in_progress", "completed")
        assert StateMachine.is_valid_transition("in_progress", "cancelled")
        assert StateMachine.is_valid_transition("in_progress", "blocked")
        assert StateMachine.is_valid_transition("blocked", "in_progress")
        assert StateMachine.is_valid_transition("blocked", "completed")
        assert StateMachine.is_valid_transition("blocked", "cancelled")
        assert StateMachine.is_valid_transition("standby", "pending")
        assert StateMachine.is_valid_transition("standby", "in_progress")
        assert StateMachine.is_valid_transition("standby", "completed")
        assert StateMachine.is_valid_transition("standby", "cancelled")
        assert StateMachine.is_valid_transition("standby", "blocked")

    def test_all_invalid_transitions(self) -> None:
        """Verify invalid transitions are rejected."""
        assert not StateMachine.is_valid_transition("completed", "pending")
        assert not StateMachine.is_valid_transition("completed", "in_progress")
        assert not StateMachine.is_valid_transition("cancelled", "pending")
        assert not StateMachine.is_valid_transition("cancelled", "in_progress")
        assert not StateMachine.is_valid_transition("in_progress", "pending")

        with pytest.raises(ValueError):
            StateMachine.validate_transition("completed", "pending")

    def test_same_status_valid(self) -> None:
        """Same status should pass silently."""
        StateMachine.validate_transition("pending", "pending")
...
class TestAutoPromotion:
    ...
    def test_parent_auto_completes(self, store: SynapsisStore) -> None:
        ...
        _init_task(store, "T-PROMO-001", status="in_progress")
        _init_task(store, "T-PROMO-C1", parent="T-PROMO-001", status="pending")
        _init_task(store, "T-PROMO-C2", parent="T-PROMO-001", status="pending")
        _init_task(store, "T-PROMO-C3", parent="T-PROMO-001", status="pending")

        # Complete first two children
        server_module.task(act="update", tid="T-PROMO-C1", sts="completed")
        server_module.task(act="update", tid="T-PROMO-C2", sts="completed")
        ...
        result = _parse(server_module.task(act="update", tid="T-PROMO-C3", sts="completed"))
        assert (
    ...
    def test_no_auto_promotion_with_open_siblings(self, store: SynapsisStore) -> None:
        ...
        _init_task(store, "T-PROMO-100", status="in_progress")
        _init_task(store, "T-PROMO-C100", parent="T-PROMO-100", status="pending")
        _init_task(store, "T-PROMO-C101", parent="T-PROMO-100", status="pending")
        result = _parse(server_module.task(act="update", tid="T-PROMO-C100", sts="completed"))
