...
def write_wiki_page(
    ...
    wiki_path = project_root / "lib" / "Wiki" / f"{wiki_rel}.md"
...
def _update_wiki_index(project_root: Path, wiki_data: dict, wiki_path: Path) -> None:
    ...
    index_path = project_root / "lib" / "Wiki" / "index.md"
    if not index_path.exists():
        return

    summary = wiki_data.get("summary", "")
    try:
        wiki_rel = wiki_path.relative_to(project_root / "lib" / "Wiki")
    except ValueError:
        wiki_rel = wiki_path
...
def _update_wiki_log(project_root: Path, wiki_data: dict, wiki_path: Path) -> None:
    ...
    log_path = project_root / "lib" / "Wiki" / "log.md"
