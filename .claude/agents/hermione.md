...
| The formatting brief contradicts `obsidian-vault-conventions.md` | Guess or improvise — use vault conventions as default, flag the contradiction to the orchestrator |
...
**Method**: Apply `obsidian-vault-conventions.md` to every file. Structure: Frontmatter → H1 → Body sections → References.
...
**Action**: Determine title, tags, aliases, destination path per `obsidian-vault-conventions.md`. Verify naming conventions (lowercase slug, no absolute paths).
...
- **No vault audit** — applies `obsidian-vault-conventions.md` to own output but does not audit or correct other vault documents. That is the vault archivist's domain.
