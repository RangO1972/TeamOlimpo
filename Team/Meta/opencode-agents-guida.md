...
- **Esempio**: `hermes`, `proteo`, `atena`

### `mode`
- **Tipo**: stringa
- **Valori possibili**: `primary` | `subagent`
- **Significato**: determina come l'agente interagisce con l'utente
  - `primary`: agente principale, l'utente può invocarlo direttamente (solo Hermes attualmente)
...
- **Valori comuni nel team**:
  - `xai/grok-code-fast-1` — veloce, efficace per compiti strutturati (Hermes, Efesto)
...
- **Configurazione globale**: definito in `opencode.json` come `default_agent: "hermes"`
...
- **Comportamento**: riceve compiti da Hermes o da altri, produce output, non comunica con l'utente
- **Invocazione**: via `@nome-agente` nei messaggi o tasto Tab

> **Nota narrativa**: nel mito, Hermes è il solo messaggero che parla agli dei e agli uomini. Gli altri abitano l'Olimpo e agiscono nel loro dominio specifico.
...
|--------|------|------|------|------|---------------------|
| **Hermes** | allow | allow | ask | allow | Orchestra: deve leggere tutto, coordinare, chiede conferma per bash |
...
L'agente può eseguire comandi bash, ma **chiede conferma** prima di farlo. Protegge da esecuzioni accidentali. Hermes e Efesto usano questo livello.

### Cosa significa `task: allow`
Solo chi deve orchestrare o collaborare può delegare. Attualmente: Hermes (orchestratore) e Atena (delega a Calliope per i nomi).
...
|---------|---------------|-----------------|
| **xai/grok-code-fast-1** | Compiti strutturati, orchestrazione, sviluppo codice. Velocità ed efficienza. | Hermes (orchestrazione), Efesto (sviluppo Python) |
...
Il processo di creazione di un nuovo membro segue un flusso preciso, orchestrato da Hermes:

### Flusso di creazione (7 step)

1. **Briefing** (Hermes): raccoglie competenze richieste e vincoli
2. **Analisi dominio** (Proteo): produce profilo di competenze in `Library/Handoff/`
...
7. **Archiviazione** (Atena): sposta handoff in `Library/Handoff/Archivio/`
...
Il flusso segue il principio di orchestrazione di Hermes:
...
2. **Verifica Hermes**: la proposta passa a Hermes, che valuta l'impatto sul sistema esistente e la coerenza con il resto della documentazione.
3. **Implementazione**: Hermes delega l'aggiornamento al responsabile competente (Calliope o Dike).
...
> **Nota di Calliope**: questo flusso non è burocrazia, è armonia. Hermes assicura che ogni cambiamento serva al tutto, non al singolo.
